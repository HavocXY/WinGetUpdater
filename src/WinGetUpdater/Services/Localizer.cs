...
public sealed class Localizer : INotifyPropertyChanged
{
    ...
    private static readonly Dictionary<string, (string De, string En)> Table = new()
    {
        ...
        // Der Rollback in der Zeile ist vom Leeren der Eingabefelder (Options.Reset)
        // getrennt: hier "Zurueckrollen", dort "Zuruecksetzen". Der Hinweis bleibt,
        // weil er die Wirkung erlaeutert und nicht die Aktion benennt.
        ["Update.Restore"] = ("Zurückrollen", "Roll back"),
        ["Update.RestoreHint"] = ("Installiert die zuletzt installierte Version ({0}) erneut.",
                                  "Reinstalls the last installed version ({0})."),
        ["Update.Restoring"] = ("wird zurückgerollt …", "rolling back …"),
        ["Update.RestoredNote"] = ("zurückgerollt auf {0}", "rolled back to {0}"),
        ["Update.RestoreFailed"] = ("Zurückrollen fehlgeschlagen (Code {0})",
        ...
        ["Shell.AllCommandsHint"] = ("Der vollständige Paketmanager: suchen, installieren, deinstallieren, " +
                                     ...
                                     "pins, configuration."),
        ["Shell.FatalTitle"] = ("WinGetUpdater konnte nicht starten", "WinGetUpdater could not start"),
        ...
        ["Winget.NotFoundHint"] = ("WinGetUpdater steuert das Programm winget.exe. Es gehört zum " +
                                   "App-Installer aus dem Microsoft Store. Installieren Sie den " +
                                   "App-Installer und starten Sie die Anwendung neu.",
                                   "WinGetUpdater drives winget.exe, which ships with the App Installer " +
